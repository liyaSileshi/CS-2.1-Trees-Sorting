...
class TestBinaryMinHeap(unittest.TestCase):
    ...
    def test_insert_and_get_one_item(self):
        ...
        heap.insert(5,1)
        assert heap.size() == 1
        assert heap.get_min()[0] == 5
        assert heap.items == [(5,1)]

    def test_insert_and_get_many_items(self):
        heap = BinaryMinHeap()
        items = [(4,9), (99,25), (9,86), (7,3), (6,29), (8,5), (3,55)]
        for index, (item, priority) in enumerate(items):
            heap.insert(item, priority)
            assert heap.size() == index + 1
            min_item = min(items[: index + 1], key= lambda t: t[1])
            assert heap.get_min() == min_item
        assert heap.size() == len(items)
        assert heap.items == [(7,3), (4,9), (8,5), (99,25),(6,29), (9,86),(3,55)]

    def test_insert_and_get_many_random_items(self):
        heap = BinaryMinHeap()
        items = [(random.randrange(0, 50), random.randrange(0, 50)) for i in range(1000)]
        for index, (item, priority) in enumerate(items):
            heap.insert(item, priority)
            assert heap.size() == index + 1
            min_item = min(items[: index + 1], key= lambda t: t[1])
    ...
    def test_insert_and_delete_many_items(self):
        ...
        for item, priority in items:
            heap.insert(item, priority)
        assert heap.size() == len(items)
        for item, priority in sorted(items, key= lambda t: t[1]):
